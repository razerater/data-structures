...
int main(){
	SimpleTest();
	std::cout << "Completed all simple tests.\n" << std::endl;

	//Uncomment this to allocate a lot of 100x100 matrices so leaks will be bigger.
	
	BatchTest(100,0.1,100,100,50);
	std::cout << "Completed all batch tests.\n" << std::endl;
	

	StudentTest();
	std::cout << "Completed all student tests.\n" << std::endl;
...
void StudentTest(){
	// test if transpose() is working
	std::cout << "Initializing and printing 6x4 matrix:" << std::endl;
	Matrix matrix(6, 4);
	for (unsigned int i = 0; i < matrix.num_rows(); i++) {
		for (unsigned int j = 0; j < matrix.num_cols(); j++) {
			matrix.set(i, j, i+j*i);
		}
	}
	std::cout << matrix << std::endl;

	std::cout << "Calling transpose() on matrix and printing it:" << std::endl;
	matrix.transpose();
	std::cout << matrix << std::endl;
}
...
void ExtraCreditTest(){
	std::cout << "Initializing and printing 5x7 matrix:" << std::endl;
	Matrix matrix(5, 7);
	for (unsigned int i = 0; i < matrix.num_rows(); i++) {
		for (unsigned int j = 0; j < matrix.num_cols(); j++) {
			matrix.set(i, j, i+j*i);
	...
	std::cout << "Calling resize(6, 4, 0) on matrix and printing it:" << std::endl;
	matrix.resize(6, 4, 0);
	std::cout << matrix << std::endl;

	std::cout << "Calling resize(7, 7) on matrix and printing it:" << std::endl;
	matrix.resize(7, 7);
	std::cout << matrix << std::endl;
}
